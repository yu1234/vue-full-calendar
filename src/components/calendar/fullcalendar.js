...
var DayTableMixin = FC.DayTableMixin = {
	...
	renderHeadDateCellHtml: function(date, colspan, otherAttrs) {
		...
		var html = view.buildGotoAnchorHtml(
				{ date: date, forceOff: this.rowCnt > 1 || this.colCnt === 1 },
				htmlEscape(date.format(this.colHeadFormat)) // inner HTML
			);
		return '' +
			'<th title="'+html.substring(6,9)+'" class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '"' +
				(this.rowCnt === 1 ?
		...
		function emptyCellsUntil(endCol) { // goes from current `col` to `endCol`
			while (col < endCol) {
				...
				if (segsBelow.length) {
					...
					moreWrap = $('<div class="fc-more-div"/>').append(moreLink);
					td.append(moreWrap);
...
TimeGrid.mixin({
	...
	generateFgSegHorizontalCss: function(seg) {
		...
		var allCoord = backwardCoord + forwardCoord;
		if (this.isRTL) {
			left = 1 - forwardCoord;
			right = backwardCoord;
		}
		...
		var allCoord = forwardCoord + backwardCoord;
		props.zIndex = seg.level + 1; // convert from 0-base to 1-based
		props.left = left * 100 + '%';
		
		if (shouldOverlap && seg.forwardPressure) {
			// add padding to the edge so that forward stacked events don't cover the resizer's icon
			props[this.isRTL ? 'marginLeft' : 'marginRight'] = 5; // 10 is a guesstimate of the icon's width
			var unit = (1 - (right + left))/2;
			right += unit;
