<template>
    <div class="ds-time-line ds-left-line">
        ...
        <div class="ds-time-line-item" v-for="(schedule,i) in extendSchedules" :key="schedule.id" v-if="schedule.show"
             @click="itemClick(schedule,i)">
                ...
                if (isArrayNotNull(schedules)) {
                    ...
                    for (let i = 0, len = cSchedules.length; i < len; i++) {
                        ...
                        if (schedule.startDate && schedule.startDate instanceof Date) {
                            startTime = schedule.startDate.getTime()
                        ...
                        if (schedule.endDate && schedule.endDate instanceof Date) {
                            endTime = schedule.endDate.getTime()
                        ...
                        if (schedule.show === undefined || schedule.show === null) {
                            schedule.show = true
                        }
                        if (this.currentDate && startTime && endTime) {
